--
-- A manifest of all the game's assets
--
manifest =
{
    scripts =
    {
        ['Actions.lua'] =
        {
            path = "code/Actions.lua"
        },
        ['Character.lua'] =
        {
            path = "code/Character.lua"
        },
        ['CharSummary.lua'] =
        {
            path = "code/CharSummary.lua"
        },
        ['ChooserState.lua'] =
        {
            path = "code/ChooserState.lua"
        },
        ['Dependencies.lua'] =
        {
            path = "code/Dependencies.lua"
        },
        ['Entity.lua'] =
        {
            path = "code/Entity.lua"
        },
        ['EntityDefs.lua'] =
        {
            path = "code/EntityDefs.lua"
        },
        ['ExploreState.lua'] =
        {
            path = "code/ExploreState.lua"
        },
        ['EquipMenuState.lua'] =
        {
            path = "code/EquipMenuState.lua"
        },
        ['FadeState.lua'] =
        {
            path = "code/FadeState.lua"
        },
        ['FrontMenuState.lua'] =
        {
            path = "code/FrontMenuState.lua"
        },
        ['InGameMenuState.lua'] =
        {
            path = "code/InGameMenuState.lua"
        },
        ['ItemMenuState.lua'] =
        {
            path = "code/ItemMenuState.lua"
        },
        ['MagicMenuState.lua'] =
        {
            path = "code/MagicMenuState.lua"
        },
        ['main.lua'] =
        {
            path = "code/main.lua"
        },
        ['Map.lua'] =
        {
            path = "code/Map.lua"
        },
        ['MoveState.lua'] =
        {
            path = "code/MoveState.lua"
        },
        ['NPCStandState.lua'] =
        {
            path = "code/NPCStandState.lua"
        },
        ['Panel.lua'] =
        {
            path = "code/Panel.lua"
        },
        ['PanelLayout.lua'] =
        {
            path = "code/PanelLayout.lua"
        },
        ['PartyMember.lua'] =
        {
            path = "code/PartyMember.lua"
        },
        ['PlanStrollState.lua'] =
        {
            path = "code/PlanStrollState.lua"
        },
        ['ProgressBar.lua'] =
        {
            path = "code/ProgressBar.lua"
        },
        ['Scrollbar.lua'] =
        {
            path = "code/Scrollbar.lua"
        },
        ['Selection.lua'] =
        {
            path = "code/Selection.lua"
        },
        ['small_room.lua'] =
        {
            path = "code/small_room.lua"
        },
        ['StateMachine.lua'] =
        {
            path = "code/StateMachine.lua"
        },
        ['StateStack.lua'] =
        {
            path = "code/StateStack.lua"
        },
        ['StatusMenuState.lua'] =
        {
            path = "code/StatusMenuState.lua"
        },
        ['Textbox.lua'] =
        {
            path = "code/Textbox.lua"
        },
        ['Trigger.lua'] =
        {
            path = "code/Trigger.lua"
        },
        ['Tween.lua'] =
        {
            path = "code/Tween.lua"
        },
        ['Animation.lua'] =
        {
            path = "code/Animation.lua"
        },
        ['Util.lua'] =
        {
            path = "code/Util.lua"
        },
        ['WaitState.lua'] =
        {
            path = "code/WaitState.lua"
        },
        ['World.lua'] =
        {
            path = "code/World.lua"
        }
    },
    textures =
    {
        ['walk_cycle.png'] =
        {
            path = "art/walk_cycle.png",
        },
        ['rpg_indoor.png'] =
        {
            path = "art/rpg_indoor.png",
        },
        ['cursor.png'] =
        {
            path = "art/cursor.png",
        },
        ['gradient_panel.png'] =
        {
            path = "art/gradient_panel.png",
        },
        ['continue_caret.png'] =
        {
            path = "art/continue_caret.png",
            scale = "pixelart"
        },
        ['xpbackground.png'] =
        {
            path = "art/xpbackground.png",
        },
        ['xpforeground.png'] =
        {
            path = "art/xpforeground.png",
        },
        ['hpbackground.png'] =
        {
            path = "art/hpbackground.png",
        },
        ['hpforeground.png'] =
        {
            path = "art/hpforeground.png",
        },
        ['mpbackground.png'] =
        {
            path = "art/mpbackground.png",
        },
        ['mpforeground.png'] =
        {
            path = "art/mpforeground.png",
        },
        ['avatar_0.png'] =
        {
            path = "art/avatar_0.png",
            scale = "pixelart"
        },
        ['avatar_1.png'] =
        {
            path = "art/avatar_1.png",
            scale = "pixelart"
        },
        ['avatar_2.png'] =
        {
            path = "art/avatar_2.png",
            scale = "pixelart"
        },
        ['quickstats.png'] =
        {
            path = "art/quickstats.png",
        },
        ['mockup.png'] =
        {
            path = "art/mockup.png",
        },
        ['scrollbar.png'] =
        {
            path = "art/scrollbar.png",
            scale = "pixelart"
        },
        ['inventory_icons.png'] =
        {
            path = "art/inventory_icons.png",
            scale = "pixelart"
        }
    }
}