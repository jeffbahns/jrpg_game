            ...
            Selection:Create
            {
                ...
                RenderItem = function(self, renderer, x, y, item)
                    if not item then
                        ...
                        renderer:DrawText2d(x + self.mSpacingX/2, y, " - ")
                    else
...
function ItemMenuState:Render(renderer)
    ...
    if not self.mInCategoryMenu then
        ...
        if selectedItem then
            local itemDef = gWorld.mItemDatabase[selectedItem.id]
